...
# -*- coding: utf-8 -*-
"""
ppt_master_hifi.py — Premium "SVG -> PPTX" high-fidelity path for PPT Pro Studio.

This is the optional **Premium** button in the PPT Pro Studio workflow. It takes the
same structured `brief.json` used by the deterministic renderer
(`ppt_studio_generate.py`) and routes it through *ppt-master*'s SVG -> native PPTX
engine, producing a vector-authored, fully editable .pptx with the highest visual
fidelity (gradients / charts / precise vector layout preserved as native shapes).
...
No network, no watermarks. MIT-0. Requires the `ppt-master` skill (MIT) installed.
"""
...
# ---------------------------------------------------------------------------
# Slide SVG builders — each returns a list of svg fragment strings
# ---------------------------------------------------------------------------
class SvgSlide:
    ...
    def _title_bar(self, title, subtitle=None, num=None):
        out = [_text(X(0.6), Y(0.4), title, PX(28), self.p["primary"], bold=True),
               rect(X(0.6), Y(1.12), X(2.2), PX(3), self.p["primary"])]
        if subtitle:
            out.append(_text(X(0.6), Y(1.22), subtitle, PX(14), self.p["secondary"]))
        if num:
            out.append(_text(X(11.8), Y(0.4), num, PX(22), self.p["muted"],
    ...
    def cover(self, d):
        ...
        if variant == "left":
            ...
            out.append(_text(X(1.1), Y(2.4), title, PX(44), self.p["primary"], bold=True))
            if sub:
                out.append(_text(X(1.1), Y(3.9), sub, PX(20), self.p["secondary"]))
        else:
            t, _ = text_block(X(1.0), Y(2.3), title, PX(46), self.p["primary"],
                              bold=True, align="center", max_w=X(11.3))
            out.append(t)
            if sub:
                s, _ = text_block(X(1.0), Y(3.9), sub, PX(20), self.p["secondary"],
                                  align="center", max_w=X(11.3))
                out.append(s)
        if badge:
            out.append(_text(X(1.0), Y(5.2), badge, PX(13), self.p["muted"],
                             align="center"))
        return out
    ...
    def section(self, d):
        out = [self._bg(),
               ...
               _text(X(0.9), Y(3.25), d.get("title", ""), PX(34), self.p["primary"],
                     bold=True)]
        if d.get("subtitle"):
            out.append(_text(X(0.9), Y(4.25), d["subtitle"], PX(14), self.p["muted"]))
        return out

    def agenda(self, d):
        out = [self._bg(), self._title_bar(d.get("title", "目录"), d.get("subtitle"))]
        items = d.get("items", [])
        for i, it in enumerate(items):
            y = Y(1.7) + i * Y(0.78)
            out.append(_text(X(0.9), y, f"{i+1:02d}", PX(20), self.p["secondary"], bold=True))
            out.append(_text(X(1.7), y, it, PX(16), self.p["text"]))
            out.append(line(X(0.9), y + Y(0.66), X(12.3), y + Y(0.66), self.p["line"], 1))
        return out
    ...
    def _bullets(self, items, x, y, w, gap=Y(0.62)):
        ...
        for i, it in enumerate(items):
            yy = y + i * gap
            out.append(rect(x, yy + PX(5), PX(5), PX(5), self.p["secondary"]))
            head, detail = (it.split("|", 1) if "|" in it else (it, ""))
            out.append(_text(x + X(0.3), yy, head, PX(15), self.p["text"], bold=True))
            if detail:
                out.append(_text(x + X(0.3), yy + PX(19), detail, PX(12), self.p["muted"]))
        return "\n".join(out)
    ...
    def two_column(self, d):
        out = [self._bg(), self._title_bar(d.get("title", ""), d.get("subtitle")),
               ...
               _text(X(0.9), Y(1.9), d.get("left_title", "左栏"), PX(16),
                     self.p["primary"], bold=True),
               _text(X(7.1), Y(1.9), d.get("right_title", "右栏"), PX(16),
                     self.p["primary"], bold=True)]
    ...
    def table(self, d):
        ...
        for j, h in enumerate(headers):
            ...
            out.append(_text(tx + j * cw + X(0.1), ty + ch * 0.18, str(h), PX(13),
                             "FFFFFF", bold=True))
        for i, row in enumerate(rows, 1):
            for j, val in enumerate(row):
                fill = self.p["surface"] if i % 2 else self.p["bg"]
                out.append(rect(tx + j * cw, ty + i * ch, cw, ch, fill))
                out.append(_text(tx + j * cw + X(0.1), ty + i * ch + ch * 0.18,
    ...
    def timeline(self, d):
        ...
        for i, m in enumerate(ms):
            ...
            out.append(_text(cx, y - Y(1.0), m.get("label", ""), PX(13),
                             self.p["primary"], bold=True, align="center"))
            out.append(_text(cx, y + Y(0.15), m.get("desc", ""), PX(11),
                             self.p["muted"], align="center"))
        return out
    ...
    def quote(self, d):
        out = [self._bg(),
               ...
               _text(X(1.5), Y(2.3), d.get("quote", ""), PX(26), self.p["text"],
                     italic=True)]
        if d.get("attribution"):
            out.append(_text(X(1.5), Y(5.2), f"— {d['attribution']}", PX(15),
                             self.p["secondary"]))
        return out
    ...
    def summary(self, d):
        ...
        if d.get("conclusion"):
            ...
            out.append(_text(X(1.1), Y(6.05), d["conclusion"], PX(15), "FFFFFF",
                             bold=True))
        return out
    ...
    def contact(self, d):
        out = [self._bg(),
               _text(X(1.0), Y(2.2), d.get("title", "联系我们"), PX(36),
                     self.p["primary"], bold=True, align="center"),
               _text(X(1.0), Y(3.4), d.get("info", ""), PX(16), self.p["text"],
                     align="center")]
        return out
...
def build_project(brief: dict, project_dir: Path, pal: dict, footer: str,
    ...
    for i, sl in enumerate(slides, 1):
        ...
        (project_dir / "svg_output" / f"page_{i:03d}.svg").write_text(
            svg, encoding="utf-8")
    # spec_lock.md — ppt-master flat export contract
    spec = f"""# spec_lock
...
"""
    ...
    # notes file for footer/page numbers (rendered into SVG instead)
    # We bake footer into each content SVG to keep converter output clean.
...
def _scripts_dir(base: Path | None) -> Path | None:
    ...
    Supports both layouts:
      - flat (bundled vendor): base/svg_to_pptx.py
      - full skill install:    base/scripts/svg_to_pptx.py
    """
    if not base:
        return None
...
def main():
    ap = argparse.ArgumentParser(description="PPT Pro Studio — Premium SVG->PPTX path")
