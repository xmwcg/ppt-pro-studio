...
const TOOLS = [
  {
    name: 'generate_ppt',
    description: 'Render a structured PPT brief (JSON) into a commercial-grade, fully editable .pptx. Supports 5 design palettes and 12 slide types (cover, section, agenda, content, two_column, table, chart, timeline, quote, image, summary, contact). No watermarks, no network.',
    inputSchema: {
      type: 'object',
      properties: {
        brief: {
...
function doGenerate(args) {
  ...
  const r = spawnSync(py, [GEN, tmp, '--out', out], { encoding: 'utf8', timeout: 120000 });
  if (r.error) throw new Error('python error: ' + r.error.message);
  if (r.status !== 0) throw new Error('render failed: ' + (r.stderr || r.stdout));
  return { ok: true, file: path.resolve(out), stdout: (r.stdout || '').trim() };
}
