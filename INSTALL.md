...
### Optional: Premium SVG→PPTX (button ⑥-B)

Renders the brief into high-fidelity vector SVG pages, then via the `ppt-master`
skill's `svg_to_pptx` into a native editable `.pptx`. Requires the `ppt-master`
skill installed at `~/.workbuddy/skills/ppt-master` (MIT).

```bash
python3 ppt-pro-studio/scripts/ppt_master_hifi.py examples/sample-brief.json \
    --out deck_hifi.pptx --keep-project ./hifi_project --style tech_dark
```
...
`--footer <text>`, `--no-page-numbers`.

---

## Prerequisites
- `python3` + `python-pptx` (pip install python-pptx) for rendering
- `node >= 18` for the MCP server
- (Optional, for button ⑥-B) the `ppt-master` skill at `~/.workbuddy/skills/ppt-master` (MIT)
...
python3 ppt-pro-studio/scripts/ppt_studio_generate.py ppt-pro-studio/examples/sample-brief.json --out /tmp/test.pptx
```
